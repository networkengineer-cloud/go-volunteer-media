...
// GetCommentTagStatistics returns statistics for all comment tags (admin only)
func GetCommentTagStatistics(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		for i, tag := range tags {
			...
			db.WithContext(ctx).Table("animal_comment_tags").
				Where("comment_tag_id = ?", tag.ID).
				Count(&usageCount)
			...
			err := db.WithContext(ctx).Table("animal_comments").
			...
			err = db.WithContext(ctx).Table("animal_comment_tags").
