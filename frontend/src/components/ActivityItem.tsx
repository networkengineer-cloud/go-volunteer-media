...
import { formatDateLabel, formatSlotRangeLabel, dayOfWeekFromIso } from '../pages/group/scheduleGrid';
  ...
  return (
    <article className={`activity-item activity-item--${item.type}`} role="article">
      ...
      <div className="activity-item__header">
        ...
        <span className={`activity-item__type-badge activity-item__type-badge--${item.type}`}>
          ...
          <span className="sr-only">
            {item.type === 'announcement' ? 'Announcement' : item.type === 'coverage_request' ? 'Coverage Request' : 'Comment'}
          </span>
        </span>
      </div>
      ...
      <div className="activity-item__footer">
        ...
        {item.type === 'coverage_request' && (
          <Link
            to={`/groups/${groupId}?view=schedule`}
            className="activity-item__action-link"
          >
            View in schedule
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
