...
variable "schedule_email_notifications_enabled" {
  type        = bool
  description = "Whether coverage-request and claim emails go out for the Schedule tab. Defaults to false so beta testers don't email the whole group before the feature is ready for everyone."
  default     = true
}

variable "coverage_requests_feed_enabled" {
  type        = bool
  description = "Whether open shift-coverage requests are surfaced as entries in the group activity feed."
  default     = true
}
